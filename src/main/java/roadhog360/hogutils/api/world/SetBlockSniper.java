...
public final class SetBlockSniper {
    ...
    ///
    /// To replace stuff like that, just use {@link net.minecraftforge.event.terraingen.ChunkProviderEvent}, or
    /// {@link net.minecraftforge.event.world.ChunkEvent}.
    public abstract static class Sniper {
        ...
        /// Only runs on SetBlock calls, doesn't run if only getMetadata was called.
        /// Currently does not run if a mod was to directly set the block using the ExtendedBlockStorage but not provide an accompanying set metadata call.
