package roadhog360.hogutils.integrity;
...
/// This class detects mods that apply mixins to HogUtils and Roadhog360 mod internals, and applies various levels of enforcement,
/// depending on configuration (see [EnforceLevel]).
///
/// Any attempts by mods to disable this detection will result in increasingly strengthened measures, up to and including
/// an explicit incompatible dependency on that mod being added. Please use supported means to interact with my mods, and/or
/// contribute your own. That benefits the whole community, while writing a bespoke mixin only benefits you and could introduce instability.
/// @author embeddedt
public class MixinTaintDetector implements IExtension {
    /// The list of class packages that are considered "internal".
    private static final List<String> TARGET_PREFIXES = ObjectArrayList.of("roadhog360", "ganymedes01.etfuturum");
    /// A method handle used to read the package-private [#mixins] field. We need this to find out
    /// about mixins before they are applied.
    private static final MethodHandle GET_MIXINS_ON_CLASS_INFO;
    public static final Logger LOG = LogManager.getLogger(Tags.MOD_ID + "-MixinTaintDetector");
    ...
    private static final Collection<String> MOD_ID_WHITELIST = ObjectArraySet.of("etfuturum","alterius", "hogutils");
    ...
    public enum EnforceLevel {
        ...
        WARN,
        /// Like [#WARN], but also throws an exception, which will usually result in the game crashing.
    ...
    static {
        ...
        EnforceLevel propertyLevel = EnforceLevel.valueOf(System.getProperty("hogutils.mixinTaintEnforceLevel", DEFAULT_ENFORCE_LEVEL.name()));
    ...
    public static void initialize() {
        LOG.info("Initializing MixinTaintDetector -- Courtesy of embeddedt, from Embeddium");
    ...
    private static Set<IMixinInfo> getPotentialMixinsForClass(ClassInfo info) {
        if(GET_MIXINS_ON_CLASS_INFO != null) {
            try {
                ...
                LOG.error("Error encountered getting mixins for class info", e);
            }
        }
    ...
    private static String getModIdByPackage(String pkg) {
        ...
        for(ModContainer container : Loader.instance().getModList()) {
            for(String packge :container.getOwnedPackages()) {
                if(pkg.startsWith(packge)) {
                    id = container.getModId();
                }
            }
        }
            ...
            if(!mixins.isEmpty()) {
                ...
                if(!illegalMixinMap.isEmpty()) {
                    if(TAINTING_MODS.isEmpty()) {
                        LOG.error("Mod mixin into Embeddium internals detected. This instance is now tainted. The Embeddium team does not provide any guarantee of support for issues encountered while such mods are installed.");
                    }
                    TAINTING_MODS.addAll(illegalMixinMap.keySet());
                    var mixinList = "[" + String.join(", ", illegalMixinMap.keySet()) + "]";
                    LOG.warn("Mod(s) {} are modifying Embeddium class {}, which may cause instability.", mixinList, name);
