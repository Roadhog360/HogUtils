...
@Mixin(Block.class)
public class MixinBlock implements ITaggableBlockItem<BlockMetaPair> {
    ...
    public synchronized void addTags(int meta, String... tags) {
        ...
        for(String tag : tags) {
            BlockTags.REVERSE_LOOKUP_TABLE.computeIfAbsent(tag, o -> new SetPair<>(new ObjectAVLTreeSet<>())).getUnlocked()
                .add(BlockMetaPair.intern((Block) (Object) this, meta));
        }
    ...
    public synchronized void removeTags(int meta, String... tags) {
        ...
        for(String tag : tags) {
            SetPair<BlockMetaPair> tagSet = BlockTags.REVERSE_LOOKUP_TABLE.get(tag);
            tagSet.getUnlocked().remove(BlockMetaPair.intern((Block) (Object) this, meta));
            if(tagSet.getUnlocked().isEmpty()) {
                BlockTags.REVERSE_LOOKUP_TABLE.remove(tag);
            }
        }
        ...
        if(!baseTags.isEmpty() || !extraTags.isEmpty()) {
            if (extraTags != null) {
                ...
                for (String tag : finalTags) {
                    InheritorHelper.addInheritedRecursive(tag, finalTags, BlockTags.INHERITOR_TABLE);
                }
